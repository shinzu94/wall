...
//            $this->post->insert_post(array( $_POST['user_id'],$_POST['post']));
            
//            return true;
            //Set some validation rules
            
            $this->form_validation->set_rules('new_post','name','trim|required');
            $this->form_validation->set_rules('user_id','name','trim|required');
            
            if($this->form_validation->run())
            {
    
                $data['value']['user_id']=$this->input->post('user_id');
                $data['value']['new_post']=$this->input->post('new_post');
                $this->post->insert_post(array('user_id'=>$_POST['user_id'],'post'=>$_POST['new_post']));
	...
	public function new_share() {
		...
		if($this->input->post('form') && $this->input->post('form') == 'main_comment_req' && $this->input->post('comment')) {
			...
			$user_id = '11';
			$user_img = asset_url().'/images/wall/user_4.jpg';
